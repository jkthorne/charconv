...
describe "UTF-16BE" do
  it "encodes and decodes ASCII" do
    ...
    encoded = Iconvcr.convert(input, "UTF-8", "UTF-16BE")
    encoded.should eq(Bytes[0x00, 0x48, 0x00, 0x65, 0x00, 0x6C, 0x00, 0x6C, 0x00, 0x6F])
    decoded = Iconvcr.convert(encoded, "UTF-16BE", "UTF-8")
  ...
  it "handles BMP characters" do
    ...
    result = Iconvcr.convert(input, "UTF-16BE", "UTF-8")
  ...
  it "round-trips supplementary characters" do
    utf8 = "😀🌍".to_slice
    encoded = Iconvcr.convert(utf8, "UTF-8", "UTF-16BE")
    decoded = Iconvcr.convert(encoded, "UTF-16BE", "UTF-8")
  ...
  it "rejects lone low surrogate" do
    ...
    expect_raises(Iconvcr::ConversionError) do
      Iconvcr.convert(input, "UTF-16BE", "UTF-8")
    end
  end
end
...
describe "UTF-16LE" do
  it "encodes and decodes ASCII" do
    input = "Hi".to_slice
    encoded = Iconvcr.convert(input, "UTF-8", "UTF-16LE")
    encoded.should eq(Bytes[0x48, 0x00, 0x69, 0x00])
    decoded = Iconvcr.convert(encoded, "UTF-16LE", "UTF-8")
    decoded.should eq(input)
  end
  ...
  it "handles supplementary characters" do
    ...
    result = Iconvcr.convert(input, "UTF-16LE", "UTF-8")
    result.should eq("😀".to_slice)
  end
end
...
describe "UTF-16 (BOM)" do
  it "decodes with BE BOM" do
    ...
    result = Iconvcr.convert(input, "UTF-16", "UTF-8")
  ...
  it "encode prepends BOM" do
    input = "A".to_slice
    encoded = Iconvcr.convert(input, "UTF-8", "UTF-16")
...
describe "UTF-32BE" do
  it "encodes and decodes ASCII" do
    input = "A".to_slice
    encoded = Iconvcr.convert(input, "UTF-8", "UTF-32BE")
    encoded.should eq(Bytes[0x00, 0x00, 0x00, 0x41])
    decoded = Iconvcr.convert(encoded, "UTF-32BE", "UTF-8")
    decoded.should eq(input)
  end
  ...
  it "handles supplementary characters" do
    ...
    result = Iconvcr.convert(input, "UTF-32BE", "UTF-8")
    result.should eq("😀".to_slice)
  end
  ...
  it "rejects surrogates" do
    ...
    expect_raises(Iconvcr::ConversionError) do
      Iconvcr.convert(input, "UTF-32BE", "UTF-8")
    end
  end
...
describe "UTF-32LE" do
  it "encodes and decodes" do
    input = "A".to_slice
    encoded = Iconvcr.convert(input, "UTF-8", "UTF-32LE")
    encoded.should eq(Bytes[0x41, 0x00, 0x00, 0x00])
    decoded = Iconvcr.convert(encoded, "UTF-32LE", "UTF-8")
    decoded.should eq(input)
  end
end
...
describe "UTF-32 (BOM)" do
  it "decodes with BE BOM" do
    ...
    result = Iconvcr.convert(input, "UTF-32", "UTF-8")
    result.should eq("A".to_slice)
  end
  ...
  it "encode prepends BOM" do
    input = "A".to_slice
    encoded = Iconvcr.convert(input, "UTF-8", "UTF-32")
...
describe "UCS-2 vs UTF-16" do
  it "UCS-2BE rejects surrogates" do
    ...
    expect_raises(Iconvcr::ConversionError) do
      Iconvcr.convert(input, "UCS-2BE", "UTF-8")
    end
  end

  it "UCS-2BE handles BMP" do
    input = Bytes[0x00, 0xE9]
    result = Iconvcr.convert(input, "UCS-2BE", "UTF-8")
    result.should eq("é".to_slice)
  end
  ...
  it "UCS-2 encode rejects cp > 0xFFFF" do
    ...
    utf8 = "😀".to_slice
    expect_raises(Iconvcr::ConversionError) do
      Iconvcr.convert(utf8, "UTF-8", "UCS-2BE")
    end
  end
...
describe "UCS-2 (BOM)" do
  it "decodes with BE BOM" do
    input = Bytes[0xFE, 0xFF, 0x00, 0x41]
    result = Iconvcr.convert(input, "UCS-2", "UTF-8")
    result.should eq("A".to_slice)
  end
...
describe "UCS-2-INTERNAL / UCS-2-SWAPPED" do
  it "UCS-2-INTERNAL uses native endianness" do
    input = "é".to_slice
    encoded = Iconvcr.convert(input, "UTF-8", "UCS-2-INTERNAL")
  ...
  it "UCS-2-SWAPPED uses opposite endianness" do
    input = "é".to_slice
    encoded = Iconvcr.convert(input, "UTF-8", "UCS-2-SWAPPED")
    {% if flag?(:little_endian) %}
      encoded.should eq(Bytes[0x00, 0xE9])
    {% else %}
      encoded.should eq(Bytes[0xE9, 0x00])
...
describe "UCS-4 (aliases for UTF-32)" do
  ...
  it "UCS-4-SWAPPED uses opposite endianness" do
    input = "A".to_slice
    encoded = Iconvcr.convert(input, "UTF-8", "UCS-4-SWAPPED")
    {% if flag?(:little_endian) %}
      encoded.should eq(Bytes[0x00, 0x00, 0x00, 0x41])
    {% else %}
      encoded.should eq(Bytes[0x41, 0x00, 0x00, 0x00])
...
describe "C99" do
  it "decodes \\u escape" do
    input = "\\u00e9".to_slice
    result = Iconvcr.convert(input, "C99", "UTF-8")
    result.should eq("é".to_slice)
  end
  ...
  it "encodes non-ASCII to \\u" do
    input = "é".to_slice
    result = Iconvcr.convert(input, "UTF-8", "C99")
    result.should eq("\\u00e9".to_slice)
  end
...
describe "JAVA" do
  it "decodes \\u escape" do
    input = "\\u00e9".to_slice
    result = Iconvcr.convert(input, "JAVA", "UTF-8")
    result.should eq("é".to_slice)
  end
  ...
  it "encodes BMP to \\u" do
    input = "é".to_slice
    result = Iconvcr.convert(input, "UTF-8", "JAVA")
    result.should eq("\\u00e9".to_slice)
  end
...
describe "UTF-7" do
  it "passes ASCII through" do
    input = "Hello".to_slice
    result = Iconvcr.convert(input, "UTF-7", "UTF-8")
    result.should eq("Hello".to_slice)
  end
  ...
  it "encodes ASCII directly" do
    input = "Hello".to_slice
    result = Iconvcr.convert(input, "UTF-8", "UTF-7")
    result.should eq("Hello".to_slice)
  end

  it "round-trips BMP characters" do
    utf8 = "é".to_slice
    encoded = Iconvcr.convert(utf8, "UTF-8", "UTF-7")
    decoded = Iconvcr.convert(encoded, "UTF-7", "UTF-8")
    decoded.should eq(utf8)
  end
...
describe "Empty input" do
  it "handles empty UTF-16BE" do
    result = Iconvcr.convert(Bytes.empty, "UTF-16BE", "UTF-8")
    result.size.should eq(0)
  end

  it "handles empty UTF-32LE" do
    result = Iconvcr.convert(Bytes.empty, "UTF-32LE", "UTF-8")
    result.size.should eq(0)
  end

  it "handles empty C99" do
    result = Iconvcr.convert(Bytes.empty, "C99", "UTF-8")
    result.size.should eq(0)
  end
end
...
describe "Encoding support" do
  it "recognizes all new encodings" do
    %w[UTF-16BE UTF-16LE UTF-16 UTF-32BE UTF-32LE UTF-32
      ...
      Iconvcr.encoding_supported?(name).should be_true, "#{name} should be supported"
    end
  end

  it "recognizes aliases" do
    Iconvcr.encoding_supported?("UNICODE-1-1").should be_true
    Iconvcr.encoding_supported?("UNICODE-1-1-UTF-7").should be_true
    Iconvcr.encoding_supported?("CSUNICODE").should be_true
    Iconvcr.encoding_supported?("CSUCS4").should be_true
  end
end
